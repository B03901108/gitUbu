...
CirMgr::readCircuit(const string& fileName) {
   ...
   for (unsigned i = 1, j; i < upbLine; ++i) if (g[i]) {
      ...
      for (colNo = 1; colNo < lineNo; ++colNo) {
         j = g[i]->fanin[colNo];
         if (!g[(j / 2)]) { g[(j / 2)] = new CirUndefGate((j / 2) * 2); tmpFL.push_back(i); }
         else if (g[(j / 2)]->getTypeStr() == "UNDEF") tmpFL.push_back(i);
         g[(j / 2)]->fanout.push_back(2 * i + (j % 2));
      }
   }
   if (tmpFL.size()) floatList.push_back(tmpFL[0]);
   for (size_t i = 1, j = tmpFL.size(); i < j; ++i)
      if (floatList.back() != tmpFL[i]) floatList.push_back(tmpFL[i]);
...
CirMgr::printFloatGates() const {
   ...
   if (x > 0) {
      ...
      for (size_t i = 0; i < x; ++i) cout << ' ' << floatList[i];
      cout << endl;
   }
   x = unusedList.size();
   if (x > 0) {
