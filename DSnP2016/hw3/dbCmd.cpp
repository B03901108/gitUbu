...
{
   // TD...
   if (cmdMgr->regCmd("DBSUm", 4, new DBSumCmd) &&
      ...
      cerr << "Registering \"init\" DBcmds fails... exiting" << endl;
      return false;
   }
}
...
DBAppendCmd::exec(const string& option)
{
   ...
   if (x == 0) return CmdExec::errorOption(CMD_OPT_MISSING, "");
   else {
      if (myStrNCmp("-Row", options[0], 2) == 0) doRow = true;
      else if (myStrNCmp("-Column", options[0], 2) != 0)
         return CmdExec::errorOption(CMD_OPT_ILLEGAL, options[0]);

      if (x == 1) return CmdExec::errorOption(CMD_OPT_MISSING, options[0]);
   }
   
   int numIn;
   size_t i, upb;
   vector<int> numIns;
   if (doRow) upb = dbtbl.nCols();
   else upb = dbtbl.nRows();

   for (i = 1; i < x; ++i) {
      if (i > upb) break;
      if (options[i] == "-") numIns.push_back(INT_MAX);
      else if (myStr2Int(options[i], numIn)) numIns.push_back(numIn);
      else {
         cerr << "Error: \"" << options[i] << "\" is not an integer!!" << endl;
         return CMD_EXEC_ERROR;
      }
   }
   for (; i<= upb; ++i) numIns.push_back(INT_MAX);
   if (doRow) dbtbl.addRow(DBRow(numIns));
   else dbtbl.addCol(numIns);
...
{  
   ...
   if (myStrNCmp("-Table", options[0], 2) == 0) {
      ...
      if (options.size() < 2) return CmdExec::errorOption(CMD_OPT_MISSING, options[0]);
      if (options.size() > 2) return CmdExec::errorOption(CMD_OPT_EXTRA, options[2]);
      if (!checkRowIdx(options[1], optSec)) return CMD_EXEC_ERROR;
      cout << dbtbl[(size_t)optSec] << endl;
   }
   else if (myStrNCmp("-Column", options[0], 2) == 0) {
      if (options.size() < 2) return CmdExec::errorOption(CMD_OPT_MISSING, options[0]);
      if (options.size() > 2) return CmdExec::errorOption(CMD_OPT_EXTRA, options[2]);
      if (!checkColIdx(options[1], optSec)) return CMD_EXEC_ERROR;
      dbtbl.printCol((size_t)optSec);
      cout << endl;
   }
   else if (checkRowIdx(options[0], optFir)) {
      if (options.size() < 2) return CmdExec::errorOption(CMD_OPT_MISSING, options[0]);
      if (options.size() > 2) return CmdExec::errorOption(CMD_OPT_EXTRA, options[2]);
      if (!checkColIdx(options[1], optSec)) return CMD_EXEC_ERROR;
      if (dbtbl[optFir][optSec] == INT_MAX) cout << "null" << endl;
      else cout << dbtbl[optFir][optSec] << endl;
   }
