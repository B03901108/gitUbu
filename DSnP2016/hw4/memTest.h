...
public:
   ...
   void newObjs(size_t n) {
      // _TD_
      for (; n > 0; --n) _objList.push_back(new MemTestObj);
   }
   // Allocate "n" number of MemTestObj arrays with size "s"
   void newArrs(size_t n, size_t s) {
      // _TD_
      MemTestObj* niceAlloc;
      try {
        niceAlloc = new MemTestObj[s];
      } catch(bad_alloc& omg) {
        return;
      }

      _arrList.push_back(niceAlloc);
      --n;
      for (; n > 0; --n) _arrList.push_back(new MemTestObj[s]);
   }
