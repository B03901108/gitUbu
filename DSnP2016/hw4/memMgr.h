...
class MemBlock
{
   ...
   bool getMem(size_t t, T*& ret) {
      // _TD_
      ret = (T*)_ptr;
      size_t legalT = toSizeT(t);
      if (size_t(_end - _ptr) < legalT) return false;
      _ptr += legalT;
      return true;
   }
...
class MemRecycleList
{
   ...
   T* popFront() {
      // _TD_
      T* cpFirst = _first;
      if (_first) _first = getNext(_first);
      return cpFirst;
   }
   ...
   void reset() {
      // _TD_
      MemRecycleList<T>* nowList = _nextList;
      stack<MemRecycleList<T>*> listStack;
      while (nowList) {
        listStack.push(nowList);
        nowList = nowList->_nextList;
      }
      while(!listStack.empty()) {
        delete listStack.top();
        listStack.pop();
      }
...
public:
   ...
   void reset(size_t b = 0) {
      ...
      // _TD_
      MemBlock<T>* nowBlock;
      MemBlock<T>* lastBlock;
      nowBlock = lastBlock = _activeBlock;
      while (nowBlock->_nextBlock) {
        lastBlock = nowBlock;
        nowBlock = nowBlock->_nextBlock;
        delete lastBlock;
      }
      nowBlock->reset();
      ...
      if ((b != _blockSize) && (b != 0)) {
        delete nowBlock;
        nowBlock = new MemBlock<T>(0, b);
        _blockSize = b;
      }
      _activeBlock = nowBlock;
   }
   // Called by new
   T* alloc(size_t t) {
      assert(t == S);
...
private:
   ...
   MemRecycleList<T>* getMemRecycleList(size_t n) {
      size_t m = n % R_SIZE;
      // _TD_
      MemRecycleList<T>* nowList = &(_recycleList[m]);
      MemRecycleList<T>* lastList = nowList;
      while (nowList) {
        if (n == nowList->_arrSize) return nowList;
        lastList = nowList;
        nowList = nowList->_nextList;
      }
      nowList = new MemRecycleList<T>(n);
      lastList->_nextList = nowList;
      return nowList;
   }
   ...
   T* getMem(size_t t) {
      ...
      if (legalT > _blockSize) {
        cerr << "Requested memory (" << legalT << ") is greater than block size("
          << _blockSize << "). " << "Exception raised...\n";
        throw(bad_alloc());
      }

      if (t > S) {
        itemSize = getArraySize(legalT);
        // itemSize = (t - SIZE_T) / S;
        ret = getMemRecycleList(itemSize)->popFront();
      } else ret = _recycleList[0].popFront();
      if (ret) {
        #ifdef MEM_DEBUG
        cout << "Recycled from _recycleList[" << itemSize << "]..." << ret << endl;
        #endif // MEM_DEBUG
        return ret;
      }
      ...
      if (!_activeBlock->getMem(legalT, ret)) {
        itemSize = _activeBlock->getRemainSize();
        if (itemSize >= toSizeT(S)) {
          itemSize = (itemSize - SIZE_T) / S;
          getMemRecycleList(itemSize)->pushFront(ret);
          #ifdef MEM_DEBUG
          cout << "Recycling " << ret << " to _recycleList[" << itemSize << "]\n";
          #endif // MEM_DEBUG
        }
        ...
        _activeBlock->getMem(legalT, ret);
      }
   ...
   size_t getNumBlocks() const {
      // _TD_
      size_t numBlock = 1;
      MemBlock<T>* nowBlock = _activeBlock->_nextBlock;
      while (nowBlock) {
        ++numBlock;
        nowBlock = nowBlock->_nextBlock;
      }
